...
export class Scrapers implements MangaScraper {
    ...
    async search(query: string, hostnames: string[] = this.randomHostnames(5)): Promise<SearchManga[]> {
        if (!hostnames.length) {
            throw new Error('There has to be at least one hostname');
        }
        let plugins = this.plugins.filter((p) => !!hostnames.find((hn) => p.handles(hn)) && !!p.search);
...
export { database };
export const scraper = new Scrapers();